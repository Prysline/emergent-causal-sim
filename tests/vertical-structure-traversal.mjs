...
assert.equal(passage.resource,'structure:stairA');
const structureManeuver=SP.traversalManeuver(st,lower,upper);
assert.deepEqual(structureManeuver.directionVector,{x:0,y:0,z:1});
assert.equal(structureManeuver.primaryResource,'structure:stairA');
assert.equal(structureManeuver.influenceNodes.length,2);
assert.equal(structureManeuver.structureId,'stairA');
