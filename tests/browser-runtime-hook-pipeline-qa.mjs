...
try{
  ...
  await page.waitForFunction(()=>window.SimEngine?.UI_RESIDENT_VIEW_VERSION&&window.SimEngine?.UI_OBSERVABILITY_CONTROLS_VERSION);
  ...
  assert.deepEqual(snapshot.after.slice(-2).map(x=>x.id),['uiObservability.render-mobile-summary','residentView.schedule'],'presentation hooks must run after simulation hooks');
  assert.deepEqual(snapshot.reset.slice(-2).map(x=>x.id),['uiObservability.reset','residentView.reset'],'presentation reset hooks must run after simulation normalization');
