...
目前 runtime marker：**v11.22.1・Editor Resident Capabilities**（`11.22.1-editor-resident-capabilities`）。
...
- `world-authoring-v2` 的 canonical positions / layers 使用 `x / y / z`，並把 Furniture under-clearance 等 concrete geometry 放回 authoring truth。舊 `world-authoring-v1` JSON 透過 explicit migration 升為 v2；legacy default dining-table 的 `.72m` under-clearance只在 migration 時轉成正式 geometry，不在 runtime 以 ID fallback 補回。獨立 `editor.html` 直接編輯同一份 canonical authoring document，Editor 的 current Z、選取工具與 dirty-state 都只是 ephemeral UI state，不 serialize，也不建立第二份 runtime truth。多層 authoring 可 import / export / round-trip，Slice E 起也可編譯進 runtime；runtime position / Tile / Spatial Node以 `z ?? 0` 作正式 identity。 D.1A 新增由 canonical Furniture / Container / Source / Resident 即時投影的 Scene Inspector：sidebar scene list、typed map markers與 selection/focus共用單一 ephemeral selection state；主模擬器 toolbar 也提供 Editor 入口。D.1B1 再把 Furniture / Container / Source / Resident lifecycle mutation集中到 pure `SimEditorAuthoringMutations` owner，所有 entity move / duplicate / delete / resident placement採 atomic clone → apply → validate → commit；`pendingOperation`、support choice、structured rejection只屬 Editor ephemeral state。D.1B2 在同一 mutation owner 上加入 desktop Pointer drag：pointer move 只投影 full-footprint / explicit support follower ghost 與 valid-invalid drop state，drop 仍呼叫同一 `moveFurniture` commit path；click/tap placement保持原 contract。這些 presentation / operation state 不寫回 authoring JSON，也不載入 runtime simulation modules。D.1C 再加入 explicit Editor→Simulator Playtest Bridge：Editor 只載入 pure `world-initializer.js` 做 runtime compatibility preflight，通過後把 canonical snapshot 寫入同 origin `sessionStorage`，並以 `?preview=editor` 明確啟動 simulator；`world.js` 的 `createInitialStateFromAuthoring(...)` 仍走同一 named initial-state pipeline。一般直接開 `index.html` 不讀 preview snapshot，Preview Reset固定重建同一 snapshot。Slice E 再把 multi-layer / non-zero Z正式接入 runtime identity：`map.tiles`維持單一 flattened index，z=0沿用 `x,y` compatibility key，non-zero z使用 `x,y,z`；same XY different Z不共享 occupancy / contact / crowding / route identity，Simulator以 ephemeral layer selector分層呈現；沒有 concrete vertical structure時不會產生跨 Z route。
...
- authored passage geometry 由 canonical Furniture geometry 與低階 compatibility edge constraint 持有；`PassageProfile` 由 `SimSpatial.getPassageProfile(state, fromNode, toNode)` 即時計算，不建立第二份 passage cache。正常 Editor 不 author `walkable / crawlOnly / PassageProfile / manual traversal flags`。
...
- **Geometry-derived Horizontal Topology**：`SimWorldAuthoring.deriveHorizontalTopology(authoring, {z})` 從 terrain/opening、concrete blocker、Furniture footprint 與 under-clearance 即時計算 structural openness、static blocker、cardinal adjacency、connected components 與 diagnostics。Editor preview、initializer diagnostics 與 runtime compatibility compilation 共用這個 interpretation；derived topology 不 serialize。`doorway` 本身提供 floor-level structural openness，實體門／家具仍可依 geometry 關閉通行。Runtime `tile.walkable / tile.furnitureIds` 暫時保留為 compiler 產物，不是 Editor authoring truth。Slice E 已把 `z` 納入 runtime Tile / Spatial Node / occupancy / route / contact / crowding identity；horizontal adjacency與surface transition仍只在同一 z，vertical traversal要等 concrete structure slice。
