...
目前 runtime marker：**v11.26.0・Vertical Structure Traversal**（`11.26.0-vertical-structure-traversal`）。
...
- **Dynamic Congestion**：`SimCrowding.getCrowdingProfile(state, agent, fromNode, toNode, mode)` 由當下 Agent occupancy、XYZ movement direction、MovementEnvelope width 與已知 PassageProfile width 即時計算；不保存 persistent crowding cache，也不修改 PassageProfile。樓梯把自己的 `clearanceWidth` 投影進同一 PassageProfile，因此窄樓梯在人多／逆向流時可沿用既有 pressure → delayTicks 模型增加實際 movement ticks，而不是硬編 `stair = 2 ticks`。
