...
目前 runtime marker：`11.23.0-world-boundary-door-exit`。
...
World v4 / Door-Opening-Exit slice 將 Grid / Wall decision gate 落成正式 contract：`map.cellSizeMeters = 1` 建立 integer grid index 與 SI metric geometry 的換算邊界；普通牆／結構開口改由 layer `boundaries` 持有，牆不再消耗整個 Cell。Door 成為 root `doors` entity，只擁有 opening boundary reference 與 state；off-map world exit 成為 root `exits` endpoint，另持有 access position。`front-door` Furniture Definition 與 `runtimeExitSlotKeys → canExit` compatibility bridge 因此從 current `furniture-definitions-v2` 移除，External Supply 改讀正式 Exit identity。`deriveHorizontalTopology(...)`、Spatial room flood-fill、PassageProfile 與 route 都讀同一 boundary interpretation；wall / closed Door 阻斷 edge，opening / open Door 保留 edge。Passage subsystem 因新增 boundary clearance / edge closure contract，換代為 `11.23.0-boundary-passage-profile`；Physical、Route Semantics、Locomotion、Crowding 等未改 contract 的 generation 不跟著假升。
