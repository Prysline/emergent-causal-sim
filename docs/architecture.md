...
- authored structure / passage geometry：`map.cellSizeMeters`、layer `boundaries`、root `structures`、Door / Exit reference、Furniture Definition 的 local `spatial.floor / spatial.under / spatial.surface`，以及可選 `map.passageConstraints`
...
Current default world 的 authored instance truth 由 `SimWorldAuthoring.DEFAULT_WORLD_AUTHORING` 持有；目前 contract generation 是 `authoringSchema: "world-authoring-v6"`，並以 `furnitureCatalogVersion: "furniture-definitions-v4"` pin 住 system-owned Furniture Catalog。Furniture Instance 的 placement truth 為 `id / definitionId / origin / orientation / optional name`，其中 `orientation ∈ { north, east, south, west }`。Current authoring owner 是 `src/world-authoring.js`；Furniture Catalog owner 是 pure `src/furniture-definitions.js`。Authoring generation、Furniture Catalog generation 與 current simulation runtime marker 分離，不能互相假升。

Authoring package 保存「這個 world instance 開場是什麼」：`map.cellSizeMeters = 1`、各 Z layer 的 floor Cell terrain / material、格線 `boundaries`、root `structures / doors / exits`、compact Furniture Instance（`id / definitionId / origin / optional name`）、Container / Source instance config與初始內容物／位置、Resident identity / traits / opening Needs / wellbeing / status與 initial placement。普通 wall / opening 不再以 Cell `terrain:'wall' / 'doorway'` 表示；Door 只引用 opening boundary 並持有 open/closed state，off-map Exit 另外持有 boundary reference 與內側 access position。Furniture 的 intrinsic name/icon/kind、local footprint/display offset、slot、under-clearance與 activity suitability由 `SimFurnitureDefinitions` 持有；canonical v5 不保存 instance-level `footprint / displayAt / slots / spatial / value / restQuality / sleepQuality / canExit`，也不保存 derived `walkable / crawlOnly / roomId / map.rooms / roomRevision / tile.furnitureIds / PassageProfile / MovementEnvelope / route / crowding` 等第二份 truth。
...
Structure edge 不借用 boundary。`SimSpatial.getPassageProfile(...)` 對合法 stair edge 產生 `edgeKind: "structure" / structureId` 並把 Structure clearance 與既有 under/edge constraint共同組合；第一版 stair 只允許既有 `walk`，不新增 `climbAbility`、slope / step-rise axis 或 stair locomotion mode。方向性客觀負擔由 system-owned stair traversal profile派生，instance 不保存 `upCost / downCost`；current additive burden為上樓 `+1`、下樓 `+0.5`，維持 `up > down >= flat`，但空樓梯仍可在 1 movement tick完成一條 edge。Spatial Traversal generation因此為 `11.26.0-vertical-structure-traversal`，Spatial Passage為 `11.26.0-vertical-structure-passage`；Route Semantics、Locomotion、Physical與Spatial Identity contract未改 generation。

Dynamic Congestion 同步把 movement direction 從 XY 擴成 XYZ，generation為 `11.26.0-vertical-flow-congestion`。樓梯 `clearanceWidth` 進入同一 PassageProfile，所以多人／逆向樓梯流可沿用既有 width pressure → congestion → `delayTicks` 增加 execution time；不硬編 stair-specific 2-tick規則，也不在本 slice新增 `flowCapacity`。
...
- `speedFactor` 已接入 execution timing：edge movement ticks = `ceil(1 / speedFactor)`，posture/mode change另加 1 transition tick；Debug、route `travelTime`與 core movement共用同一 timing truth。`speedFactor` 不直接換算 objective burden。
- **Locomotion objective burden**：`SimLocomotion.modeTraversalBurden(agent, mode)` current baseline 為 walk=0、kneelCrawl=1、proneCrawl=2（每 edge）；`modeTransitionBurden(agent, fromMode, toMode)` 在 mode 改變時為 1、同 mode 為 0。這是 coarse 客觀移動麻煩程度，不是人格／禮儀偏好，也不是 `travelTime` 的別名。
