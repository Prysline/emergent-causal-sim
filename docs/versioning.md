...
World authoring 另有獨立 contract generation：current `SimWorldAuthoring.VERSION = "world-authoring-v6"`，canonical package 同時保存 `authoringSchema: "world-authoring-v6"`、`furnitureCatalogVersion: "furniture-definitions-v5"`、`map.cellSizeMeters = 1`、root `structures`，以及 Furniture Instance required `orientation`。Furniture Catalog 自己由 `SimFurnitureDefinitions.VERSION` 持有 generation；它不是 current runtime marker，也不是 Physical / Spatial 等 simulation subsystem generation。Current source owner 使用 semantic path `src/world-authoring.js` 與 `src/furniture-definitions.js`。

只有 authoring package shape / compatibility需要新 generation時才升 `world-authoring-vN`；單純 current runtime推進不得順手假升 authoring schema。歷史 Slice D 因 Furniture under-clearance進入 canonical v2 geometry而升 v2；Editor-2 因 Furniture intrinsic facts與 instance placement正式拆成 Definition / Instance ownership而升 v3；v4 因 `cellSizeMeters`、grid-line `boundaries`、root Door / Exit entity 與 Cell terrain ownership改變而再升一代；v5 因新增獨立 root `structures` 與明確 vertical Structure compatibility contract 再升一代；current v6 因 Furniture Instance 新增 required `orientation` 而換代。因產品尚未正式公開，current-only v6 不提供舊 generation production migration machinery。Furniture Catalog generation只有 catalog contract本身改變時才升；World v4 因移除 `front-door` / exit compatibility contract曾升為 `furniture-definitions-v2`，Furniture Traversal Geometry 再因新增 Definition-owned `spatial.floor / spatial.surface` contract並移除 Furniture `blocksMovement` truth 升為 `furniture-definitions-v3`，Furniture Orientation 則因 canonical local-frame / resolver transform contract 升為 `furniture-definitions-v4`；Room Value Legacy Removal 再因移除 Definition `roomValueContribution` 與 resolver legacy `value` projection 升為 current `furniture-definitions-v5`。v11.27 中 Physical、Route、Locomotion、Spatial Identity仍未換代；Spatial Traversal、Spatial Passage與Crowding維持 v11.26 已建立的 `11.26.0-vertical-structure-traversal`、`11.26.0-vertical-structure-passage`、`11.26.0-vertical-flow-congestion`。
