...
World authoring 另有獨立 contract generation：current `SimWorldAuthoring.VERSION = "world-authoring-v3"`，canonical package 同時保存 `authoringSchema: "world-authoring-v3"` 與 `furnitureCatalogVersion: "furniture-definitions-v1"`。Furniture Catalog 自己由 `SimFurnitureDefinitions.VERSION` 持有 generation；它不是 current runtime marker，也不是 Physical / Spatial 等 simulation subsystem generation。Current source owner 使用 semantic path `src/world-authoring.js` 與 `src/furniture-definitions.js`。

只有 authoring package shape / compatibility需要新 generation時才升 `world-authoring-vN`；單純 current runtime推進不得順手假升 authoring schema。歷史 Slice D 因 Furniture under-clearance進入 canonical v2 geometry、並需要 explicit v1→v2 migration，所以 authoring contract曾升為 v2；Editor-2 則因 Furniture intrinsic facts與 instance placement正式拆成 Definition / Instance ownership而升為 v3。因產品尚未正式公開，current-only v3 不提供 v2→v3 production migration machinery。Furniture Catalog generation只有 catalog contract本身改變時才升，不因 overall runtime patch自動跟升。Physical / Passage、Route、Locomotion、Crowding各自 contract沒有換代，因此維持原 subsystem marker。
