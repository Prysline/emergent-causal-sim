...
`11.14.4-entity-readable-view`

玩家可見標題使用短版 `v11.14.4`；`state.version`、`SimWorld.PRESENTATION_SCHEMA_VERSION`、Resident View 與 Entity Readable View 的 UI version 使用完整 marker。
...
像 `presentation-schema-v1140.js`、`ui-resident-view-v1140.js`、`ui-entity-readable-v1141.js`、`browser-resident-view-v1140-qa` 這類名稱代表 subsystem / test family，可以跨同一 product line 的 patch 延續，不需要每個 patch 都複製／改名整組檔案與 workflow。判斷目前版本時，以 `state.version`、`SimWorld.PRESENTATION_SCHEMA_VERSION`、玩家可見 app version 與 Current 文件為準，而不是從檔名或 workflow display name 反推 current release。
...
3. `SimEngine.UI_RESIDENT_VIEW_VERSION` 與 `SimEngine.UI_ENTITY_READABLE_VERSION`（若已載入）都與 schema marker 一致；
