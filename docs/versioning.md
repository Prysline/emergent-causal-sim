...
World authoring 另有獨立 contract generation：current `SimWorldAuthoring.VERSION = "world-authoring-v4"`，canonical package 同時保存 `authoringSchema: "world-authoring-v4"`、`furnitureCatalogVersion: "furniture-definitions-v2"` 與 `map.cellSizeMeters = 1`。Furniture Catalog 自己由 `SimFurnitureDefinitions.VERSION` 持有 generation；它不是 current runtime marker，也不是 Physical / Spatial 等 simulation subsystem generation。Current source owner 使用 semantic path `src/world-authoring.js` 與 `src/furniture-definitions.js`。

只有 authoring package shape / compatibility需要新 generation時才升 `world-authoring-vN`；單純 current runtime推進不得順手假升 authoring schema。歷史 Slice D 因 Furniture under-clearance進入 canonical v2 geometry而升 v2；Editor-2 因 Furniture intrinsic facts與 instance placement正式拆成 Definition / Instance ownership而升 v3；current v4 因 `cellSizeMeters`、grid-line `boundaries`、root Door / Exit entity 與 Cell terrain ownership改變而再升一代。因產品尚未正式公開，current-only v4 不提供 v3→v4 production migration machinery。Furniture Catalog generation只有 catalog contract本身改變時才升；本次因移除 `front-door` / exit compatibility contract升為 `furniture-definitions-v2`。Physical、Route、Locomotion、Crowding等未換代 subsystem維持原 marker；Passage因新增 boundary clearance / edge closure contract，獨立升為 `11.23.0-boundary-passage-profile`。
