(() => {
  ...
  function validateLayer(st,base){
    ...
    for(const f of Object.values(st?.furniture||{})){
      const under=f.spatial?.under;if(!under)continue;
      if(under.clearance!=null&&!positive(under.clearance))add('spatial_passage_height_invalid',`${f.name} 的 under.clearance 必須是正數或未設定。`,{furnitureId:f.id,value:under.clearance});
      if(under.clearanceWidth!=null&&!positive(under.clearanceWidth))add('spatial_passage_width_invalid',`${f.name} 的 under.clearanceWidth 必須是正數或未設定。`,{furnitureId:f.id,value:under.clearanceWidth});
    }
