  ...
  function validateLayer(st,base){
    ...
    for(const e of Object.values(st?.causes||{})){
      ...
      if(d.action==='petOffer'){
        ...
        const actor=st.agents?.[d.actor],target=st.agents?.[d.target];
        if(actor?.kind!=='human'||!target||E.isAnimalAgent?.(target)!==true||E.canPetAnimal?.(actor,target)!==true)add('pet_offer_affordance_invalid',`petOffer ${e.id} 必須由 human 指向目前可撫摸的 animal target。`,{eventId:e.id,actor:d.actor,target:d.target});
        if(d.perceivedByTarget!==true)add('pet_offer_perception_invalid',`petOffer ${e.id} 只應在 awake animal 實際可感知時建立。`,{eventId:e.id});
      }
      if(RESPONSE_ACTION[d.action]){
        ...
        else if(offer.data.bidFrom!==d.target||offer.data.bidTo!==d.actor)add('pet_response_direction_mismatch',`事件 ${e.id} 的 animal response 方向與 petOffer 不一致。`,{eventId:e.id,bidId:offer.id});
        if(d.responseToBid){
      ...
      if(d.action==='petAnimal'&&d.petOfferId){
        ...
        if(!offer||offer.data?.bidKind!=='petOffer')add('pet_success_offer_missing',`petAnimal ${e.id} 引用的 petOffer ${d.petOfferId} 不存在。`,{eventId:e.id,petOfferId:d.petOfferId});
        else if(offer.data.bidFrom!==d.actor||offer.data.bidTo!==d.target)add('pet_success_direction_mismatch',`petAnimal ${e.id} 的 actor/target 與 petOffer 不一致。`,{eventId:e.id,petOfferId:d.petOfferId});
        if(!['accept','tolerate'].includes(d.petResponse))add('pet_success_response_invalid',`petAnimal ${e.id} 只能由 accept/tolerate response 產生。`,{eventId:e.id,petResponse:d.petResponse});
    ...
    for(const [offerId,responses] of responseByOffer){
      if(responses.length!==1)add('pet_offer_response_count_invalid',`petOffer ${offerId} 應只有一個 immediate animal response。`,{bidId:offerId,count:responses.length});
      const response=responses[0],pets=petByOffer.get(offerId)||[];
      if(response?.data?.petResponse==='avoid'&&pets.length)add('pet_avoid_success_conflict',`petOffer ${offerId} 已被 avoid，不可同時產生成功 petAnimal。`,{bidId:offerId,petCount:pets.length});
      if(['accept','tolerate'].includes(response?.data?.petResponse)&&pets.length!==1)add('pet_accept_success_missing',`petOffer ${offerId} 的 ${response.data.petResponse} response 應對應一個成功 petAnimal。`,{bidId:offerId,petCount:pets.length});
    }
