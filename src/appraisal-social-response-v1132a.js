(() => {
  ...
  function appraiseAvoidPetMemory(st,a,memory){
    ...
    let relevance=.08,goalCongruence=0,factors=[{kind:'bystander',relevanceDelta:.08}];

    if(o.actorId===a.id){
      const social=normNeed(a.needs?.social);
      relevance=.55;
      goalCongruence=round(.20+.45*(1-social));
      factors=[
        {kind:'selfActor',relevanceDelta:.25},
        {kind:'declinedUnwantedContact',key:'social',level:round(social),relevanceDelta:.30,congruenceDelta:goalCongruence}
      ];
    }else if(o.targetId===a.id){
      const affinity=clamp(Number(a.traits?.animalAffinity)||0,0,1);
      const extraRelevance=round(.10+.15*affinity),negative=round(.18+.32*affinity);
      relevance=round(.35+extraRelevance);
      goalCongruence=-negative;
      factors=[
        {kind:'directTarget',relevanceDelta:.35},
        {kind:'petOfferDeclined',key:'animalAffinity',level:round(affinity),relevanceDelta:extraRelevance,congruenceDelta:-negative}
      ];
    }
  ...
  E.onEpisodicMemoryCreated=(st,a,memory)=>{
    ...
    if(memory?.observed?.action==='avoidPet')return appraiseAvoidPetMemory(st,a,memory);
