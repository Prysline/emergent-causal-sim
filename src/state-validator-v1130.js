  ...
  function validateState(st){
    ...
    for(const a of Object.values(st?.agents||{})){
      ...
      for(const m of memories){
        ...
        for(const key of ['text','causeIds','appraisal','affect','salience','utilityInfluence'])if(Object.prototype.hasOwnProperty.call(m,key))add('episodic_memory_out_of_scope_field',`${a.name} 的 v11.13.0 memory 不應提前保存 ${key}。`,{agentId:a.id,memoryId:m.id,key});
      }
    }
