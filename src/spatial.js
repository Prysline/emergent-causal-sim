(() => {
  ...
  function recomputeRooms(st){
    ...
    for(const t of Object.values(tiles)){
      ...
      while(queue.length){const cur=queue.shift();floors.push(cur.id);for(const [dx,dy] of dirs){const n=tileAt(st,cur.x+dx,cur.y+dy);if(isRoomFloor(n)&&!n.roomId){n.roomId=id;queue.push(n);}}}
      const floorSet=new Set(floors),wallSet=new Set(),furnitureSet=new Set();
      for(const tid of floors){const ft=tiles[tid];for(const fid of ft.furnitureIds||[])furnitureSet.add(fid);for(const [dx,dy] of dirs){const n=tileAt(st,ft.x+dx,ft.y+dy);if(n?.terrain==='wall'||n?.terrain==='doorway')wallSet.add(n.id);}}
  ...
  function neighbors(st,p){const out=[];for(const [dx,dy] of [[1,0],[-1,0],[0,1],[0,-1]]){const n={x:p.x+dx,y:p.y+dy};if(walkable(st,n))out.push(n);}return out;}
  ...
  function adjacentWalkable(st,p){const out=[];for(const [dx,dy] of [[1,0],[-1,0],[0,1],[0,-1]]){const q={x:p.x+dx,y:p.y+dy};if(walkable(st,q))out.push(q);}return out;}
