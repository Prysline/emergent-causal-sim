(() => {
  ...
  function decorateInspector({host,selected,state:st}){
    if(!host||host.querySelector('[data-v1135-social-outcome-memory]')||selected?.type!=='agent')return;
  ...
  if(!UI?.registerInspectorDecorator)throw new Error('socialOutcome.memory requires inspector decorator lifecycle');
  UI.registerInspectorDecorator('socialOutcome.memory',decorateInspector,900);
})();
