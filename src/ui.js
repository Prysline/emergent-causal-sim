(() => {
  ...
  function renderActions(){
    ...
    $('actions').innerHTML=Object.values(st.agents).map(a=>{
      ...
      return `<article class="action-card" data-entity="${entityKey('agent',a.id)}">
        <div class="action-top"><div class="avatar">${a.kind==='cat'?'🐈':'👤'}</div><div><div class="action-name">${a.name}</div><div class="action-location">📍 ${S.zoneName(a.location)}${a.held?`・拿著 ${S.endpointName(a.held)}`:''}</div></div></div>
        <div class="action-now">▶ ${esc(S.planLabel(a))}</div>
  ...
  function worldOverview(){
    ...
    return `<div class="empty-inspector"><div><span>🔎</span><b>點選任何東西</b><br>角色、杯子、酒瓶、水龍頭、地面與區域都可以檢查。<br><br>目前 ${Object.keys(st.agents).length} 名 Agent・${Object.keys(st.containers).length} 個容器・${wet} 個區域有液體殘留。</div></div>`;
  }
  ...
  function displayValue(k,v){if(typeof v==='number')return Math.round(v*100)/100;if(k==='resource')return S.resourceName(v);if(k==='location'||(k==='target'&&state().zones[v]))return S.zoneName(v);if(['from','to','container','source'].includes(k)){if(v==='internal')return'體內';if(state().zones[v])return S.zoneName(v);return S.endpointName(v)}return S.ZH[v]||v}
  ...
  function inspectContainer(id){
    ...
    const props=[c.portable?propTag('可攜帶','good'):propTag('固定物件'),c.canDrink?propTag('可直接飲用','info'):'',c.refillFrom?propTag(`可補充：${S.endpointName(c.refillFrom)}`,'info'):''].filter(Boolean).join('');
  ...
  function inspectAgent(id){const st=state(),a=st.agents[id],th=st.thoughts[id],contacts=Object.entries(a.contacts).flatMap(([part,c])=>Object.entries(c).filter(([,v])=>v>.1).map(([r,v])=>`${part==='paws'?'腳掌':part==='hands'?'雙手':'腳'}：${S.resourceName(r)} ${Math.round(v)}`));let decision='';if($('showThoughts').checked&&th){decision=`<div class="decision">${th.options.slice(0,7).map(o=>`<div class="decision-row"><span>${S.ZH[o.id]||o.id}${o.id===th.pick.id?' ← 選擇':''}</span><b>${Math.round(o.score)}</b></div>`).join('')}<div class="decision-reason"><b>${S.ZH[th.pick.id]||th.pick.id}</b><br>${th.pick.why.map(x=>'＋ '+esc(x)).join('<br>')}</div></div>`}else decision='<div class="timeline-empty">尚未產生決策資料，或已關閉決策原因。</div>';
    return `<div class="inspect-title"><div class="inspect-icon">${a.kind==='cat'?'🐈':'👤'}</div><div><h2>${a.name}</h2><small>Agent・${id}</small></div></div><div class="inspect-section"><h3>現在</h3><div class="kv"><div class="k">位置</div><div>${S.zoneName(a.location)}</div><div class="k">行動</div><div>${esc(S.planLabel(a))}</div><div class="k">醉酒</div><div>${Math.round(a.status.intoxication)}</div><div class="k">動作協調</div><div>${Math.round(S.coordination(a))}</div><div class="k">持有</div><div>${a.held?S.endpointName(a.held):'無'}</div></div></div><div class="inspect-section"><h3>需求</h3><div class="tags">${Object.entries(a.needs).map(([k,v])=>propTag(`${S.ZH[k]||k} ${Math.round(v)}`,v>=75?'bad':v>=55?'warn':'' )).join('')}</div></div>${contacts.length?`<div class="inspect-section"><h3>附著物</h3><div class="tags">${contacts.map(x=>propTag(x,'warn')).join('')}</div></div>`:''}<div class="inspect-section"><h3>最近決策</h3>${decision}</div><div class="inspect-section"><h3>最近相關事件</h3>${recentBlock('agent',id)}</div>`}
  ...
  function render(){const st=state();$('clock').textContent=`第 ${st.day} 天 ${S.timeStr()}`;$('tickLabel').textContent=`Tick ${st.tick}`;renderWorldBadges();renderMap();renderActions();renderTimeline();renderInspector()}
  ...
  $('reset').onclick=()=>{if(timer){clearInterval(timer);timer=null;$('play').textContent='▶ 開始'}S.reset();selected=null;render()};
