(() => {
  ...
  function worldOverview(validationSnapshot){const s=st(),v=validationSnapshot,wet=Object.values(s.map.tiles).filter(t=>SP.tileLiquidAmount(t)>.1).length;return `<div class="empty-inspector"><div><span>🔎</span><b>點選任何東西</b><br>角色、物件、家具、Room、事件與 Tile 都可以檢查。<br><br>目前 ${Object.keys(s.agents).length} 名 Agent・${Object.keys(s.map.rooms).length} 個 Room・${wet} 格濕地。<br>Seed：${s.seed}<br>Core：${esc(s.version)}${v.issueCount?`<br><br><b>⚠ 狀態檢查：${v.issueCount}</b><br>${v.issues.slice(0,3).map(x=>esc(x.message)).join('<br>')}`:''}</div></div>`;}
