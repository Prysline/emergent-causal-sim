(() => {
  ...
  function normalizeAgentAffect(a,tick=0){
    ...
    const f=a.affect;
    f.valence=round(clamp(Number(f.valence)||0,-1,1));
    f.activation=round(clamp(Number(f.activation)||0,0,1));
    f.frustration=round(clamp(Number(f.frustration)||0,0,1));
  ...
  function decayAgentAffect(a,nextTick){
    ...
    f.activation=shrink(f.activation,DECAY.activation,false);
    f.frustration=shrink(f.frustration,DECAY.frustration,false);
    f.lastDecayTick=nextTick;
    if(f.valence===0&&f.activation===0&&f.frustration===0)f.source=null;
    return f;
  }
  ...
  function updateAffectFromAppraisal(st,a,memory){
    ...
    const relevance=clamp(Number(p.relevance)||0,0,1),congruence=clamp(Number(p.goalCongruence)||0,-1,1);
    const positiveImpact=relevance*Math.max(0,congruence),negativeImpact=relevance*Math.max(0,-congruence),magnitude=positiveImpact+negativeImpact;
    if(magnitude<=0)return normalizeAgentAffect(a,st?.tick||0);
    const f=normalizeAgentAffect(a,st?.tick||0);
    f.valence=round(clamp(f.valence+(positiveImpact-negativeImpact)*.55,-1,1));
    f.activation=round(clamp(f.activation+magnitude*.45,0,1));
    f.frustration=round(clamp(f.frustration+negativeImpact*.65-positiveImpact*.15,0,1));
    f.lastUpdatedTick=st?.tick||0;
    f.source={memoryId:memory.id,sourceEventId:memory.sourceEventId,appraisedTick:p.appraisedTick,appliedTick:st?.tick||0};
    return f;
  }

  E.onEpisodicMemoryCreated=(st,a,memory)=>{
    if(typeof priorMemoryCreatedHook==='function')priorMemoryCreatedHook(st,a,memory);
    updateAffectFromAppraisal(st,a,memory);
