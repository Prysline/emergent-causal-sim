(() => {
  ...
  function randomReachableFloor(st,a){const choices=Object.values(st.map?.tiles||{}).filter(t=>SP.walkable(st,t)).map(t=>({t,d:SP.pathDistance(st,a,t)})).filter(x=>Number.isFinite(x.d)&&x.d>0);if(!choices.length)return null;return choices[Math.min(choices.length-1,Math.floor(E.rand(0,choices.length)))]?.t||null;}
  function makeAction(st,a,actionKindValue,{targetAgent=null}={}){
    ...
    switch(actionKindValue){
      ...
      case'drinkWater':case'drinkAlcohol':{const resource=actionKindValue==='drinkWater'?'water':'alcohol';if(a.kind==='cat'){const src=nearestDrinkContainer(st,a,resource);return src?{...base,phase:'move',targetObject:src.id,resource}:null;}return {...base,phase:'chooseVessel',resource};}
      case'rest':return {...base,phase:'chooseSurface',restTicks:0};case'sleep':return {...base,phase:'chooseSurface',sleepTicks:0};
  ...
  function bindActionToIntent(a,intent,action){if(!action)return false;E.installActionKind?.(action);action.intentId=intent.id;intent.lifecycle='actionBound';a.action=action;a.activeIntent=intent;return true;}
  ...
  function startEmergency(st,a,c){const priorActionKind=actionKind(a),priorIntent=clone(a.activeIntent),priorLabel=priorActionKind?(E.ZH?.[priorActionKind]||priorActionKind):priorIntent?`「${E.intentLabel?.(priorIntent)||priorIntent.kind}」`:'目前安排';cleanupForInterruption(st,a);a.activeIntent=null;const action=makeAction(st,a,c.actionKind);if(!action)return false;E.installActionKind?.(action);const intent=E.createIntent?E.createIntent(st,a,action):{id:`intent:${a.id}:${st.tick}:${c.intentKind}`,kind:c.intentKind,createdTick:st.tick,lifecycle:'actionBound',source:{type:'emergency',tick:st.tick}};intent.kind=c.intentKind;intent.lifecycle='actionBound';intent.source={type:'emergency',need:c.need,value:c.value,tick:st.tick};action.intentId=intent.id;a.action=action;a.activeIntent=intent;E.addEvent(`${a.name}因${E.ZH?.[c.need]||c.need}過於迫切，中斷${priorLabel}，改先處理緊急需求。`,'warn',[],{actor:a.id,action:'intentPreempt',priorActionKind:priorActionKind||null,nextActionKind:c.actionKind,intentId:intent.id,intentKind:intent.kind,emergencyNeed:c.need,emergencyValue:c.value,priorIntentId:priorIntent?.id||null,position:E.positionRef(a.position)});return true;}
