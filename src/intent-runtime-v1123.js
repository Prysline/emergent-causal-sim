(() => {
  ...
  function startEmergency(st,a,c){const priorActionKind=actionKind(a),priorIntent=clone(a.activeIntent),priorLabel=priorActionKind?(E.ZH?.[priorActionKind]||priorActionKind):priorIntent?`「${E.intentLabel?.(priorIntent)||priorIntent.kind}」`:'目前安排';cleanupForInterruption(st,a);a.activeIntent=null;const action=buildAction(st,a,c.actionKind);if(!action)return false;E.installActionKind?.(action);const intent=E.createIntent?E.createIntent(st,a,action):{id:`intent:${a.id}:${st.tick}:${c.intentKind}`,kind:c.intentKind,createdTick:st.tick,lifecycle:'actionBound',source:{type:'emergency',tick:st.tick}};intent.kind=c.intentKind;intent.lifecycle='actionBound';intent.source={type:'emergency',need:c.need,value:c.value,tick:st.tick};action.intentId=intent.id;a.action=action;a.activeIntent=intent;E.addEvent(`${a.name}因${E.ZH?.[c.need]||c.need}過於迫切，中斷${priorLabel}，改先處理緊急需求。`,'warn',[],{actor:a.id,action:'intentPreempt',priorActionKind:priorActionKind||null,nextActionKind:c.actionKind,intentId:intent.id,intentKind:intent.kind,emergencyNeed:c.need,emergencyValue:c.value,priorIntentId:priorIntent?.id||null,position:E.positionRef(a.position)});return true;}
